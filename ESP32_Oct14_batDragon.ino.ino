...
#define EVEN_BRIGHTNESS 64   // Brightness for even-indexed LEDs
#define ODD_BRIGHTNESS  125  // Brightness for odd-indexed LEDs
...
void fadeToColor(CRGB fromColor, CRGB toColor, uint16_t fadeTime) {
  ...
  while (millis() < endTime) {
    ...
    for (uint16_t i = 0; i < NUM_LEDS; i++) {
      // Set brightness based on the LED index
      if (i % 2 == 0) {
        leds[i] = targetColor;
        leds[i].fadeToBlackBy(EVEN_BRIGHTNESS);
      } else {
        leds[i] = targetColor;
        leds[i].fadeToBlackBy(ODD_BRIGHTNESS);
      }
    }
